...
export function genReadme(projectName, arch, state, feats) {
  const buildRunner = state === 'bloc' || state === 'riverpod' || feats.includes('codegen')
...
This project contains the **Dart & Architecture** structure but requires native folders (android, ios, etc.) to be generated.
...
\`\`\`bash
flutter create . --org com.example --project-name ${snakeCaseName}
\`\`\`
*(This will generate the android, ios, windows, and web directories automatically)*
...
\`\`\`bash
${buildRunner ? `# 1. Generate code (freezed, injectable, etc.)
flutter pub run build_runner build --delete-conflicting-outputs

` : ''}# ${buildRunner ? '2' : '1'}. Run the app
flutter run
\`\`\`
